...
const StatsCard = ({ title, value, icon: Icon, trend, color, index = 0, onClick, className }) => {
    return (
        <motion.div
            ...
            onClick={onClick}
            className={cn(
                "bg-white dark:bg-slate-900 rounded-[24px] sm:rounded-[32px] p-5 sm:p-8 shadow-sm border border-slate-200 dark:border-slate-800 flex flex-col gap-4 sm:gap-6 group hover:shadow-xl transition-all duration-500 relative overflow-hidden",
        ...
        >
            ...
            <div className={cn(
                "w-11 h-11 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl flex items-center justify-center transition-all duration-300 group-hover:scale-110 shadow-lg",
                ...
                <Icon size={24} className="hidden sm:block" strokeWidth={2.5} />
            </div>

            <div className="space-y-1">
                <p className="text-[9px] sm:text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">{title}</p>
                <div className="flex items-center gap-2 sm:gap-3">
                    <h3 className="text-2xl sm:text-4xl font-bold text-slate-900 dark:text-white tracking-tight">{value}</h3>
                    {trend && (
                        <span className={cn(
                            "text-[8px] sm:text-[10px] font-black px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-lg uppercase tracking-wider",
